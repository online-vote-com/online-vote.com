...
$pdo->prepare("INSERT INTO logs_paiement(transaction_id,payload) VALUES(?,?)")
    ->execute([$data['externalReference'] ?? null, $content]);

if (!isset($data['externalReference'], $data['status'])) { http_response_code(400); exit; }
...
if ($status === "SUCCESS") {
    ...
    if (!$paiement || $paiement['status_paiement'] === 'succes') exit;
    ...
    for ($i=0; $i < $paiement['quantite_vote']; $i++) {
        $pdo->prepare("INSERT INTO votes (id_candidat, id_concours, id_paiement, source) VALUES (?,?,?,'payant')")
            ->execute([$paiement['id_candidat'], $paiement['id_concours'], $paiement['id_paiement']]);
    }
}
